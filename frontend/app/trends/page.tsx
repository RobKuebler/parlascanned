          ...
          >
            ...
            <div className="flex flex-col gap-2">
              <div className="flex flex-wrap gap-1.5 items-center">
                <span
                  className="text-[11px] uppercase tracking-wide mr-1"
                  style={{ color: "#9A9790" }}
                >
                  Fraktion
                </span>
                {/* "Alle" pill */}
                <button
                  onClick={() => setSelectedParties(null)}
                  className="px-3 py-1 rounded-full text-[12px] transition-colors"
                  style={{
                    background:
                      selectedParties === null ? "#1E1B5E" : "#F0EEE9",
                    color: selectedParties === null ? "#fff" : "#9A9790",
                    fontWeight: selectedParties === null ? 600 : 400,
                  }}
                >
                  Alle
                </button>

                {/* Fallback: show skeleton pills while party data isn't known yet */}
                {parties.length === 0
                  ? ["CDU/CSU", "SPD", "AfD", "Grüne", "FDP"].map((name) => (
                      <button
                        key={name}
                        onClick={() => ensurePartyData()}
                        className="px-3 py-1 rounded-full text-[12px] transition-colors"
                        style={{ background: "#F0EEE9", color: "#9A9790" }}
                      >
                        {name}
                      </button>
                    ))
                  : parties.map((party) => {
                      const isActive = selectedParties?.has(party) ?? false;
                      const color = PARTY_COLORS[party] ?? "#9A9790";
                      ...
                      return (
                        <button
                          ...
                          className="px-3 py-1 rounded-full text-[12px] transition-colors"
                          style={{
                            background: isActive ? color + "18" : "#F0EEE9",
                            color: isActive ? color : "#9A9790",
                            border: isActive
                              ? `1px solid ${color}50`
