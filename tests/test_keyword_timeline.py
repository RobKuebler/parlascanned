...
def test_compute_keyword_timeline_by_party_counts(sample_df):
    ...
    """
    result = kt.compute_keyword_timeline(
        sample_df, stopwords=set(), min_count=1, party_breakdown_top_n=9999
    )
    bp = result["by_party"]
...
def test_compute_keyword_timeline_by_party_top_n(sample_df):
    """by_party contains only the party_breakdown_top_n most frequent terms."""
    result = kt.compute_keyword_timeline(
        sample_df, stopwords=set(), min_count=1, party_breakdown_top_n=2
    )
...
def test_compute_keyword_timeline_fraktionslos_excluded(sample_df):
    ...
    result = kt.compute_keyword_timeline(
        df, stopwords=set(), min_count=1, party_breakdown_top_n=9999
    )
